...
const Stats: React.FC = () => {
    ...
    const getAllStats = async () => {
      try {
        const res = await axios.get(
          `${process.env.NEXT_PUBLIC_SERVER}/dashboard/dashboardStatus`,
        );
        console.log(res);
        if (res.status === 200) {
  ...
  return (
    <>
      ...
      <Link
        href="/register"
        className="mt-8 flex justify-center rounded bg-primary px-18 py-4 font-medium text-gray hover:bg-opacity-90"
      >
